...
import CRCNN.data_pro as pro
import CRCNN.pyt_CR_CNN as pa
import json
DW = 50
NDW = 100
NN = 123
...
N_BATCH_SIZE = 80
epochs = 100
target_dict = json.load(open('/home/dm/adg/remotetest/CRCNN/target_dict.txt', 'r', encoding='utf-8'))
c_target_dict = {value: key for key, value in target_dict.items()}
tr_target_dict = json.load(open('/home/dm/adg/remotetest/CRCNN/tr_target_dict.txt', 'r', encoding='utf-8'))

data = pro.load_data('/home/dm/adg/remotetest/CRCNN/nine_train.txt')
t_data = pro.load_data('/home/dm/adg/remotetest/CRCNN/nine_test.txt')
...
for i in range(epochs):
    ...
    for (b_x_cat, b_y) in eval_dataloader:
        ...
        ti += 1
    print('epoch:', i, 'f1:', acc / j, '%   loss:', loss.cpu().data.numpy()[0] / j, 'test_f1:', eval_acc / ti, '%')
